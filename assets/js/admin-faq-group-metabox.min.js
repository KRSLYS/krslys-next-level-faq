/******/ (() => { // webpackBootstrap
/*!**********************************************!*\
  !*** ./assets/js/admin-faq-group-metabox.js ***!
  \**********************************************/
(() => {
  'use strict';
  ...
  function initNewEditor(row) {
    ...
    const editorAPI = window.wp.oldEditor && typeof window.wp.oldEditor.initialize === 'function' ? window.wp.oldEditor : window.wp.editor && typeof window.wp.editor.initialize === 'function' ? window.wp.editor : null;
    if (editorAPI) {
  ...
  function restoreEditors(row, editorData) {
    ...
    const editorAPI = window.wp && window.wp.oldEditor && typeof window.wp.oldEditor.initialize === 'function' ? window.wp.oldEditor : window.wp && window.wp.editor && typeof window.wp.editor.initialize === 'function' ? window.wp.editor : null;
    if (!editorAPI) {
      return;
    }
  ...
  function showTitleError() {
    ...
    titleInput.addEventListener('animationend', () => titleInput.classList.remove('nlf-shake'), {
      once: true
    });
    titleInput.focus();
    ...
    if (!wrap.querySelector('.nlf-field-error')) {
      ...
      msg.innerHTML = '<span class="dashicons dashicons-warning" aria-hidden="true"></span><span>' + (nlfGroupData.i18n?.title_required || 'Please enter a title for this FAQ group.') + '</span>';
  ...
  function initCopyButtons() {
    $$('.nlf-htu-snippet').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (!text) return;
        const write = navigator.clipboard && navigator.clipboard.writeText ? navigator.clipboard.writeText(text) : new Promise(resolve => {
